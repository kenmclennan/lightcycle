...
def ready_task_roles(tasks):
    ...
    return [t.role for t in tasks if t.role and t.role != "human"]


def ready_roles(tasks):
    out = []
    for role in ready_task_roles(tasks):
        if role not in out:
            out.append(role)
    return out


def pool_plan(roles, inflight, slots):
    """Decide which roles to spawn this tick for the agent pool.

    roles is the role per ready task (one entry each, repeats allowed). inflight
