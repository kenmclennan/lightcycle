...
def _file_compat(argv):
    ...
    nargs = ["item", title, "--description", "a description"]
    for k, flag in (("project", "--project"), ("goal", "--goal"),
                    ("workflow", "--workflow")):
        if opts.get(k):
            nargs += [flag, opts[k]]
...
class TestTitleCap(unittest.TestCase):
    ...
    def test_new_item_title_over_cap_is_rejected_end_to_end(self):
        rc, out, err = call(_cli_mod.cmd_new, "item", "x" * 100, "--description", "a description")
...
class TestModel(unittest.TestCase):
    ...
    def test_task_mapping_and_status(self):
        bid = create_owned_step(self.store, "build: thing", step="build", role="agent")
    ...
    def test_status_lanes_json(self):
        h = create_owned_step(self.store, "spec: x", step="spec", role="human")
        c = create_owned_step(self.store, "build: y", step="build", role="agent")
...
class TestClaim(unittest.TestCase):
    ...
    def test_claim_ignores_human(self):
        create_owned_step(self.store, "spec: x", step="spec", role="human")
...
class TestRun(unittest.TestCase):
    ...
    def test_run_pool_fills_up_to_max_agents(self):
        for i in range(7):
            create_owned_step(self.store, "build: %d" % i, step="build", role="agent")
        for i in range(3):
            create_owned_step(self.store, "review: %d" % i, step="review", role="agent")
        rc, _, err = self._run_once()
        self.assertEqual(rc, 0, err)
...
class TestAdd(unittest.TestCase):
    ...
    def test_new_item_with_shortcodeless_project_refuses_and_creates_nothing(self):
        ...
        rc, out, err = call(_cli_mod.cmd_new, "item", "look at X later", "--project", "acme/ghost", "--description", "a description")
        self.assertEqual(rc, 1)
...
class TestFileItem(unittest.TestCase):
    ...
    def test_file_creates_story_with_spec_and_build_task(self):
        rc, out, err = call(_file_compat, "specs/HSS-435.md", "--step", "build", "--workflow", "lightcycle/spec-driven")
        self.assertEqual(rc, 0, err)
        sid = out.strip()
        self.assertTrue(sid)
...
class TestFileBlockedBy(unittest.TestCase):
    ...
    def test_blocked_by_creates_dependency_on_first_task(self):
        gate = create_owned_step(self.store, "review-plan: foo", step="review-plan", role="human")
        rc, out, err = call(
            _file_compat, "specs/X.md", "--step", "build", "--workflow", "lightcycle/spec-driven", "--blocked-by", gate
        )
        self.assertEqual(rc, 0, err)
        sid = out.strip()
        self.assertTrue(sid)
    ...
    def test_multiple_blocked_by_ids(self):
        gate1 = create_owned_step(self.store, "gate1", role="human")
        gate2 = create_owned_step(self.store, "gate2", role="human")
        rc, out, _ = call(
            _file_compat,
            "specs/X.md",
            "--step",
...
class TestClaimArtifacts(unittest.TestCase):
    ...
    def test_claim_surfaces_artifact_replacement(self):
        rc, out, err = call(_file_compat, "specs/Y.md", "--step", "build", "--workflow", "lightcycle/spec-driven")
        self.assertEqual(rc, 0, err)
        item = out.strip()
...
class TestFileStep(unittest.TestCase):
    ...
    def test_file_rejects_unknown_step(self):
        rc, _, err = call(_file_compat, "specs/X.md", "--step", "bogus", "--workflow", "lightcycle/spec-driven")
        self.assertNotEqual(rc, 0)
...
class TestSpecsWorkspaceWorktree(unittest.TestCase):
    ...
    def test_claim_creates_a_worktree_inside_the_specs_repo_not_the_project(self):
        ...
        self.assertEqual(t["description"], "the settled design")

    def test_activating_a_spec_workflow_item_without_a_repo_fails_fast(self):
        item = self.store.create_item("phase b1", "a description", workflow="lightcycle/spec-driven")
...
class TestNamedRepo(unittest.TestCase):
    ...
    def _claim(self, repo=None):
        args = ["specs/X.md", "--step", "build", "--workflow", "lightcycle/spec-driven"]
        if repo:
            args += ["--repo", repo]
    ...
    def test_activating_an_item_with_an_unresolvable_named_repo_fails_loudly(self):
        rc, out, err = call(
            _file_compat, "specs/X.md", "--step", "build", "--workflow", "lightcycle/spec-driven", "--repo", "ghost"
        )
        self.assertEqual(rc, 1)
...
class TestActivationClonesAnAbsentRegisteredProject(unittest.TestCase):
    ...
    def test_activate_clones_a_registered_project_with_no_local_checkout(self):
        ...
        item = self.store.create_item("add refunds", "a description")
...
class TestUnblock(unittest.TestCase):
    ...
    def test_unblock_refuses_human_step(self):
        ...
        b = create_owned_step(self.store, "ready-merge: t", step="ready-merge", role="human")
        rc, out, err = call(_cli_mod.cmd_set, b, "--state", "ready")
        self.assertEqual(rc, 1)
...
class TestClose(unittest.TestCase):
    ...
    def test_close_item_closes_and_force_closes_its_open_steps(self):
        ...
        self.assertEqual(self.store.get_node(step).state, "done")
        self.assertEqual(self.store.get_node(step).outcome, "wontfix")
...
class TestInboxBacklog(unittest.TestCase):
    ...
    def test_inbox_limit_n(self):
        create_owned_step(self.store, "merge: p", step="ready-merge", role="human")
        create_owned_step(self.store, "merge: q", step="ready-merge", role="human")
        create_owned_step(self.store, "merge: r", step="ready-merge", role="human")
...
class TestReflect(unittest.TestCase):
    ...
    def _file_story(self, spec_path=None):
        sid = self.store.create_item("feat", "a description", workflow="lightcycle/spec-driven")
...
class TestRetro(unittest.TestCase):
    ...
    def _item(self, sid=None):
        if sid is None:
            sid = self.store.create_item("item-1", "a description", workflow="lightcycle/spec-driven")
        return sid
    ...
    def test_retro_rejects_zero_or_multiple_selectors(self):
        ...
        sid = self._item()
        rc, out, err = call(_cli_mod.cmd_retro, sid, "--pending")
        self.assertEqual(rc, 2)
        self.assertIn("provide exactly one of", err)
...
class TestCadenceStepDTO(unittest.TestCase):
    ...
    def test_show_cadence_task_includes_since(self):
        tid = create_owned_step(self.store, "trend: window", step="audit", role="agent")
...
class TestResumeFieldsSurfaceOnShow(unittest.TestCase):
    ...
    def test_show_surfaces_the_park(self):
        tid = create_owned_step(self.store, "build: t", step="build", role="human")
...
class TestWorkflowFieldNeverGoesMissing(unittest.TestCase):
    ...
    def test_show_includes_workflow_key_when_unset(self):
        tid = create_owned_step(self.store, "build: t", step="build", role="agent")
        rc, out, err = call(_cli_mod.cmd_show, tid)
        self.assertEqual(rc, 0, err)
...
class TestWorkflowSelection(unittest.TestCase):
    ...
    def _file(self, spec, workflow, *args):
        _, out, err = call(_file_compat, spec, "--workflow", workflow, *args)
        return out.strip()
    ...
    def test_two_items_route_by_their_own_workflow(self):
        ...
        solo_item = self._file("B.md", "lightcycle/solo")

        call(_cli_mod.cmd_done, self._build_task(std_item).id, "done")
        self.assertIn("review", self._open_successor_steps(std_item))

        call(_cli_mod.cmd_done, self._build_task(solo_item).id, "done")
        self.assertEqual(self._open_successor_steps(solo_item), set())
