Feature: The stats screen
  ...
  list). It shows five store-wide figures for one day - what closed, what it
  cost, the backlog's size and delta, how many audits, how many escalations -
  defaulting to today. It has its own independent day picker, shared with the
  done tab's own. Pressing Enter opens the done tab filtered to whichever day
  stats is currently showing, for the per-item breakdown.
  ...
  Scenario: The stats tab shows five figures for today by default
    Given the store has a closed item today, costing money, with a completed disposition
    When I switch to the stats tab
    Then the stats figures show 1 closed item, 1 completed and 0 aborted
    And the stats figures show a recorded cost
  ...
  Scenario: Selecting a day updates the figures and the day filter bar
    Given the store has closed items on two distinct days before today
    When I switch to the stats tab
    And d is pressed
    And Down is pressed
    ...
    And the stats figures show 1 closed item, 1 completed and 0 aborted
