Feature: Breaker recovers when its probe worker stalls
  ...
  treated as a successful probe. A probe that is still alive but has already produced
  real session activity is recognised as successful the moment that activity appears,
  without waiting for it to finish or die. The breaker's own reset time is re-armed forward by a
  ...
  Scenario: A stalled probe whose log shows a terminal marker is a success, not a stall
    Given the breaker is open and past its reset time
    And the probe worker is alive and its log has stalled
  ...
  Scenario: A live probe that has produced session activity closes the breaker without waiting for it to die
    Given the breaker is open and past its reset time
    And the probe worker is alive and has produced session activity
    When the pool's breaker gate runs
    Then the breaker closes

  Scenario: A live probe with a pending rejection is not treated as a successful probe
    Given the breaker is open and past its reset time
    And the probe worker is alive and its log carries a rate-limit rejection
    When the pool's breaker gate runs
    Then the breaker does not close
    And the reset time is not re-armed
