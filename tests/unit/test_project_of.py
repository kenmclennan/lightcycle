...
class TestProjectOf(unittest.TestCase):
    def test_an_item_node_reads_project_directly_without_calling_get_item(self):
        s = FakeStore()
        item_id = s.create_item("item", "a description", project="org/proj")
...
class TestRepoOf(unittest.TestCase):
    def test_an_item_node_reads_repo_directly_without_calling_get_item(self):
        ...
        s.get_item = _fail
        self.assertEqual(repo_of(s, item), "org/proj")
    ...
    def test_a_step_node_resolves_via_its_parent_item(self):
        ...
        step_id = s.create_step(step="write-code", parent=item_id)
        step = s.get_step(step_id)
        self.assertEqual(repo_of(s, step), "org/proj")

    def test_a_bare_string_item_id_resolves_via_get_item(self):
        s = FakeStore()
        item_id = s.create_item("item", "a description")
        s.add_artifact(item_id, "repo", "org/proj")
        self.assertEqual(repo_of(s, item_id), "org/proj")

    def test_an_unknown_string_id_returns_none(self):
        s = FakeStore()
        self.assertIsNone(repo_of(s, "no-such-id"))
