...
class TestDashboardScaffold(unittest.TestCase):
    ...
    def test_priority_list_is_not_truncated_to_ten(self):
        store = FakeStore()
        ids = [create_owned_step(store, "t%d" % i, step="build", role="agent") for i in range(12)]
...
class TestNeedsAttentionGroup(unittest.TestCase):
    ...
    def test_inbox_row_renders_above_active_and_queued(self):
        store = FakeStore()
        inbox = create_owned_step(store, "inbox item", step="triage", role="human")
        active = create_owned_step(store, "active item", step="build", role="agent")
    ...
    def test_inbox_row_shows_single_icon_and_current_step_styled_amber(self):
        store = FakeStore()
        inbox = create_owned_step(store, "inbox item", step="code-await-merge", role="human")
    ...
    def test_needs_attention_row_with_long_title_wraps(self):
        store = FakeStore()
        step = create_owned_step(store, "word " * 60, step="triage", role="human")
...
class TestActiveGroup(unittest.TestCase):
    ...
    def test_active_glyph_animation_does_not_run_without_an_active_row(self):
        store = FakeStore()
        create_owned_step(store, "queued item", step="build", role="agent")
...
class TestQueuedGroup(unittest.TestCase):
    ...
    def test_queued_row_renders_below_active_with_own_icon_and_next_step(self):
        ...
        active = create_owned_step(store, "active item", step="build", role="agent")
        store.assign(active, "worker-1")
        queued = create_owned_step(store, "queued item", step="build", role="agent")
    ...
    def test_dependency_held_step_appears_in_queued_group_with_dim_chain_glyph(self):
        ...
        blocked = create_owned_step(store, "blocked item", step="build", role="agent", deps=[blocker])
...
class TestProjectColumn(unittest.TestCase):
    ...
    def test_step_with_no_registered_project_renders_blank(self):
        store = FakeStore()
        step = create_owned_step(store, "build", step="build", role="agent")
...
class TestCursorColumn(unittest.TestCase):
    ...
    def test_selected_row_shows_leading_cursor_glyph(self):
        ...
        create_owned_step(store, "second", step="build", role="agent")
    ...
    def test_cursor_glyph_follows_the_selection(self):
        store = FakeStore()
        first = create_owned_step(store, "first", step="build", role="agent")
        second = create_owned_step(store, "second", step="build", role="agent")
...
class TestEmptyState(unittest.TestCase):
    ...
    def test_message_is_replaced_once_a_row_appears(self):
        ...
        create_owned_step(store, "new", step="build", role="agent")
        session.poll_tick()
...
def _report_rows_shown(session):
...
class _CountingReportUseCase:
    def __init__(self):
        self.calls = 0

    def patcher(self):
        real_execute = tui_app_module.ReportUseCase.execute
        counts = self
        ...
        return patch.object(tui_app_module.ReportUseCase, "execute", counted)


class TestReportTabSwitch(unittest.TestCase):
    def _launch(self, store=None):
        session = launch(make_test_container(store=store or FakeStore()))
        self.addCleanup(session.close)
        return session
...
class TestReportGatedOffThePoll(unittest.TestCase):
    def test_polling_while_on_priority_never_computes_report(self):
        store = FakeStore()
        session = launch(make_test_container(store=store))
        self.addCleanup(session.close)
        self.assertEqual(session.app._view, "priority")
    ...
    def test_polling_while_on_done_never_computes_report(self):
        store = FakeStore()
        session = _launch_done(store)
        self.addCleanup(session.close)
        self.assertEqual(session.app._view, "done")
    ...
    def test_switching_to_report_triggers_exactly_one_execute_call(self):
        store = FakeStore()
        session = launch(make_test_container(store=store))
        self.addCleanup(session.close)
    ...
    def test_poll_ticks_while_on_report_today_do_not_recompute(self):
        store = FakeStore()
        session = _launch_report(store)
        self.addCleanup(session.close)
        counter = _CountingReportUseCase()
    ...
    def test_revisiting_a_past_day_does_not_recompute_but_today_always_does(self):
        ...
        app._report_day = day_a
        session.run(app._refresh_report_view)
        session.pause()
...
class TestReportDayPicker(unittest.TestCase):
    def _launch_two_days(self):
        store = FakeStore()
        older = store.create_item("older item", "a description")
    ...
    def test_selecting_a_day_updates_the_figures_and_day_filter_bar(self):
        ...
        self.assertNotIsInstance(session.app.screen, ProjectFilterPicker)
        self.assertEqual(session.app._report_day, datetime.date(2026, 1, 1))
        left = session.app.query_one("#report-day-filter-left", Static)
        self.assertEqual(_rendered_text(left).strip(), "2026-01-01")
        self.assertEqual(_report_rows_shown(session)[0], ("Items Completed", "1"))
        self.assertEqual(_report_rows_shown(session)[1], ("Items Closed", "1"))


class TestReportOpensDoneForDay(unittest.TestCase):
    def test_enter_on_report_opens_done_filtered_to_the_shown_day(self):
        store = FakeStore()
        older = store.create_item("older item", "a description")
        store.complete_node(older, "merged")
        store._records[older]["closed_at"] = "2026-01-01T10:00:00+00:00"
    ...
    def test_enter_is_a_no_op_outside_report(self):
        session = launch(make_test_container(store=FakeStore()))
        self.addCleanup(session.close)
