...
class TestCreateItemFixtureMode(unittest.TestCase):
    def test_default_fake_mints_an_opaque_id_when_no_shortcode_is_given(self):
        iid = FakeStore().create_item("item: foo", "a description")
        self.assertTrue(iid.startswith("fake-"), iid)
