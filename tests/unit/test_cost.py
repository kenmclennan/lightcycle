...
class _FakeStep:
    def __init__(self, stage="write-code", role="agent", turn_count=0, usage_cost_usd=0.0,
...
class TestStepCost(unittest.TestCase):
    def test_human_role_short_circuits_regardless_of_turn_or_cost_data(self):
        step = _FakeStep(role="human", turn_count=5, usage_cost_usd=5.0, usage_cost_basis="list")
        cost = step_cost(step, {})
    ...
    def test_agent_ran_recorded_has_every_figure(self):
        step = _FakeStep(role="agent", turn_count=10, usage_cost_usd=1.5, usage_cost_basis="list")
        cost = step_cost(step, {})
        self.assertTrue(cost.has_run)
        self.assertTrue(cost.recorded)
        self.assertAlmostEqual(cost.cost_per_turn, 0.15)

    def test_tools_ordered_by_calls_descending_tool_name_ascending_on_tie(self):
        step = _FakeStep(role="agent", turn_count=10, usage_cost_usd=1.0, usage_cost_basis="list")
        tool_usage = {
...
class TestItemCost(unittest.TestCase):
    def test_human_step_contributes_nothing(self):
        steps = [_FakeStep(role="human", turn_count=1, usage_cost_usd=5.0, usage_cost_basis="list")]
    ...
    def test_basis_counts_distinguish_list_derived_and_not_recorded(self):
        steps = [
            _FakeStep(role="agent", turn_count=1, usage_cost_usd=1.0, usage_cost_basis="list"),
            _FakeStep(role="agent", turn_count=2, usage_cost_usd=2.0, usage_cost_basis="derived"),
            _FakeStep(role="agent", turn_count=3, usage_cost_usd=0.0, usage_cost_basis=None),
        ]
        result = item_cost(steps)
    ...
    def test_cost_per_turn_divides_by_recorded_turn_count_not_full_total(self):
        steps = [
            _FakeStep(role="human", turn_count=100, usage_cost_usd=0.0),
            _FakeStep(role="agent", turn_count=10, usage_cost_usd=3.0, usage_cost_basis="list"),
            _FakeStep(role="agent", turn_count=246, usage_cost_usd=0.0, usage_cost_basis=None),
        ]
        result = item_cost(steps)
        self.assertEqual(result.recorded_turn_count, 10)
        self.assertAlmostEqual(result.cost_per_turn, 0.3)

    def test_per_stage_subtotal_ordered_by_spend_descending(self):
        steps = [
            _FakeStep(stage="cleanup", role="agent", turn_count=2, usage_cost_usd=0.17, usage_cost_basis="list"),
            _FakeStep(
                stage="spec-writer", role="agent", turn_count=10, usage_cost_usd=2.91, usage_cost_basis="list",
            ),
        ]
        result = item_cost(steps)
    ...
    def test_stage_tie_on_spend_breaks_by_stage_name_ascending(self):
        steps = [
            _FakeStep(stage="write-code", role="agent", turn_count=1, usage_cost_usd=1.0, usage_cost_basis="list"),
            _FakeStep(stage="cleanup", role="agent", turn_count=1, usage_cost_usd=1.0, usage_cost_basis="list"),
        ]
        result = item_cost(steps)
    ...
    def test_stage_whose_steps_have_not_run_yet_still_appears(self):
        steps = [_FakeStep(stage="build", role="agent", turn_count=0, usage_cost_usd=0.0)]
        result = item_cost(steps)
        self.assertEqual(len(result.stages), 1)
        self.assertEqual(result.stages[0].step_count, 1)
        self.assertEqual(result.stages[0].turn_count, 0)
        self.assertEqual(result.stages[0].cost_usd, 0.0)

    def test_stage_not_recorded_count_only_counts_steps_that_have_run(self):
        steps = [
            _FakeStep(stage="review-code", role="agent", turn_count=0, usage_cost_usd=0.0),
            _FakeStep(stage="review-code", role="agent", turn_count=60, usage_cost_usd=0.0, usage_cost_basis=None),
        ]
        result = item_cost(steps)
