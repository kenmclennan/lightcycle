...
class TestPoolPlan(unittest.TestCase):
    def test_fills_up_to_slots_in_queue_order(self):
        self.assertEqual(
            PoolPlan({}, 2).roles_to_spawn(["coder", "coder", "coder", "reviewer"]),
            ["coder", "coder"],
        )

    def test_preserves_role_mix(self):
        self.assertEqual(
            PoolPlan({}, 5).roles_to_spawn(["coder", "reviewer", "coder"]),
            ["coder", "reviewer", "coder"],
        )

    def test_inflight_worker_covers_a_ready_task(self):
        self.assertEqual(
            PoolPlan({"coder": 1}, 5).roles_to_spawn(["coder", "coder", "reviewer"]),
            ["coder", "reviewer"],
        )
