...
def _resolve_agent(cfg, role, pin):
    return WorkflowSourceAdapter(cfg, WorkflowBundleAdapter()).resolve_agent(role, pin)
...
class TestResolveAgentForPin(unittest.TestCase):
    def test_pin_none_resolves_from_prompts_root(self):
        ...
        agent = _resolve_agent(_Cfg(prompts, data), "audit", None)
    ...
    def test_pin_none_does_not_search_any_bundle(self):
        ...
        self.assertIsNone(_resolve_agent(_Cfg(prompts, data), "write-code", None))
    ...
    def test_unknown_role_returns_none(self):
        prompts, data = tempfile.mkdtemp(), tempfile.mkdtemp()
        _write_bundle_prompt(data, "lightcycle", "sha1", "write-code")
        self.assertIsNone(_resolve_agent(
