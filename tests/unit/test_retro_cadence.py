...
def _gate(store, interval_items=3):
    return RetroCadenceUseCase(store, FakeConfig(interval_items))
...
class TestRetroCadenceNoFire(unittest.TestCase):
    ...
    def test_below_threshold_does_not_fire(self):
        ...
        self.assertEqual(_gate(s, interval_items=3).execute(0.0).fired, [])
...
class TestRetroCadenceFires(unittest.TestCase):
    def test_threshold_fires_one_global_audit(self):
        ...
        for i in range(3):
            _close_item(s, "item %d" % i, reflection="fb")
        result = _gate(s, interval_items=3).execute(0.0)
    ...
    def test_fires_the_fixed_engine_audit_step_and_role(self):
        ...
        for i in range(3):
            _close_item(s, "item %d" % i, reflection="fb")
        step = s.get_node(_gate(s, interval_items=3).execute(0.0).fired[0])
    ...
    def test_item_without_repo_but_with_reflection_counts(self):
        ...
        for i in range(3):
            _close_item(s, "item %d" % i, reflection="fb")
        self.assertEqual(len(_gate(s, interval_items=3).execute(0.0).fired), 1)
