...
def _backup_store(store, data_root):
    ...
    dst = os.path.join(backups, _NEW_DB + ".gz")
...
def migrate_legacy(config):
    ...
    new_store = os.path.join(data_root, _NEW_DB)
    if os.path.exists(new_store):
        return MigrateResponse(already=True)

    legacy_root = config.legacy_data_root()
    legacy_store = os.path.join(legacy_root, _OLD_DB)
    legacy_config = os.path.join(legacy_root, "config")
    ...
    for name in ("steps", "workflows", "logs", ".worktrees"):
