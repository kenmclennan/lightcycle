...
1. CLAIM: `lc claim auditor`. If nothing, say "no work" and EXIT. Take `.id` as TASK and
   `.epic` as EPIC.
2. Read the epic's retro digest: `lc show EPIC` and locate the `retro` artifact. Its value
...
1. CLAIM: `lc claim auditor`. If nothing, say "no work" and EXIT. Take `.id` as TASK and
   `.since` as SINCE (from the task's `since` metadata field - check `lc show TASK`).
2. Run the cross-epic retro: `lc retro --since SINCE`
