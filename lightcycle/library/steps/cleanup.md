...
1. `lc done STORY merged` - closes the story + its child tasks (status closed, reason merged),
...
2. `lc done TASK done` is not needed - cleanup is terminal and `lc done` closes the task.
