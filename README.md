...
Work is **items** and **steps**, in their own tables with no shared shape ("node" survives only as a loose word for either), and the CLI is a small set of generic primitives over them (`new`/`set`/`show`/`done`/`rm`/`attach`/`dep`, plus `claim`). The workflow supplies the meaning; the engine ships no per-workflow verbs.
...
- **Hierarchy: item / step.** An **item** is the unit you plan and ship (one spec -> one branch -> one PR), the top of the tree, and **holds the artifacts**; a **step** is one stage of the item's workflow running (or the reusable step definition). An item's state is **derived** from its steps, never stored: `backlogged` (captured, nothing filed) -> `in_progress` (steps running) -> `done`. `lc new item` captures it backlogged; `lc set <item> --state active` **plans** it - filing the workflow's entry step, and first ensuring the item's repo is cloned if the registry knows it but it isn't checked out here yet.
...
- **Labels route work:** `for:<role>` (`agent`, `human`, or `engine`), `step:<step>` (flow stage), `project:`/`goal:`. `for:human` steps never auto-run; they surface to you. `for:engine` steps are never spawned either - the engine completes them itself.
...
Because selection is per item, two items can run different workflows concurrently. To customise or add a workflow, author a **source** (the plugin's `author-workflow` skill guides it) and `lc workflow add` it - you never edit files inside the engine.
...
| `lc set <id> [--state/--title/--description/--workflow/--project/--label/--step/--notes/--needs/--reason/--tried/--backlog/--unset]` | update a node. The type decides which flags and states are legal, and `lc set` refuses at the boundary rather than silently ignoring one: an **item** takes `title description project workflow label backlog step` and the states `active` (plans it - files its entry step) and `backlogged` (removes its open steps; refused while a worker holds a step or an open run has a branch or PR); a **step** takes `title notes needs reason tried label` and the states `ready` and `waiting`, and `waiting` requires `--needs` and `--reason`. An empty string is never a value - it is refused rather than silently applied; `--unset <field>` (repeatable) clears `description`/`project`/`workflow` on an item or `notes` on a step instead |
...
| `lc close <item> [--outcome/--disposition/--note]` | end an **item** outright; `--outcome` is free-form text (default `closed`), `--disposition` defaults to `completed`; refuses a **step** - use `lc done` for one |
| `lc rm <id>` | delete a node |
| `lc attach <id> <type> <value> [--label/--replace/--file]` | attach an artifact to an **item** (`spec`, `repo`, `brief`, `blueprint`, ...). `pr`, `branch` and `comments-handled` are not artifacts - they route onto the item's current phase run. `reflection` routes to the retro |
