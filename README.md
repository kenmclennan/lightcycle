...
| `lc set <id> [--state/--title/--description/--workflow/--project/--label/--step/--notes/--needs/--reason/--tried/--backlog]` | update a node. The type decides which flags and states are legal, and `lc set` refuses at the boundary rather than silently ignoring one: an **item** takes `title description project workflow label backlog step` and the states `active` (plans it - files its entry step) and `in_progress`; a **step** takes `title notes needs reason tried label` and the states `ready` and `blocked`, and `blocked` requires `--needs` and `--reason` |
