...
1. CLAIM: `tg claim pr-watcher`. If nothing, say "no work" and EXIT. The printed JSON is your task;
   take `.id` as BEAD, `.parent` as STORY, read `.story_artifacts` for branch (type=branch) and
   pr (type=pr).
...
5. NEVER merge. CI green + comments resolved -> `tg done BEAD done`. CI failed (code needs changing)
   -> `tg done BEAD ci-failed` (reworks on the same branch/PR). Human decision needed ->
   `tg block BEAD --needs "<...>"`.
