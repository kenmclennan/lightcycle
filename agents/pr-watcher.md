...
1. CLAIM: `tg claim pr-watcher`. If nothing, say "no work" and EXIT. The printed JSON is your task;
   take `.id` as TASK, `.parent` as STORY, `.workspace` as WORKSPACE, read `.story_artifacts` for
   branch (type=branch) and pr (type=pr).
...
3. Find or open the PR - NEVER a duplicate. If a `pr` artifact exists, use that PR (re-check its CI;
   ...
   reuse it.
4. Read CI accurately: fetch the actual failing job/logs before concluding.
5. Comments: correct -> escalate a fix; wrong -> reply refuting with evidence.
6. NEVER merge. CI green + comments resolved -> `tg done TASK done`. CI failed (code needs changing)
