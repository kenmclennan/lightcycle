...
| `audit-shortcode` / `summary-shortcode` | id prefixes for items the engine creates for itself - `audit-shortcode` (seeded `AUD`) for retro audits, `summary-shortcode` (seeded `SUM`) for daily summaries and goal state-of-play runs - distinct from `shortcode`, which prefixes items you create. Forward-only: existing ids keep the prefix they were minted with. |
