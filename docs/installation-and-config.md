...
| `audit-shortcode` / `summary-shortcode` | id prefixes for items the engine creates for itself - `audit-shortcode` (seeded `AUD`) for retro audits, `summary-shortcode` (seeded `SUM`) for daily summaries - distinct from `shortcode`, which prefixes items you create. Forward-only: existing ids keep the prefix they were minted with. |
...
| `suspend-pressure` | threshold above which the memory gate signals a running worker to suspend, checked against the pool's own memory share (the pool's footprint as a fraction of RAM), the same on every platform; the same signal also withholds new worker admission whenever it is already at or above this threshold, so a newly admitted worker is not immediately suspended; the gate never suspends the last working (unsuspended) worker, and admits one when every alive worker is suspended; must be in `[0, 1]` |
| `resume-pressure` | threshold below which the memory gate signals a suspended worker to resume, checked against the same pool memory share as `suspend-pressure`; must be set strictly below `suspend-pressure`, and must itself be in `[0, 1]` |
...
Schema changes are handled separately: when a new engine opens a store it stamps a current store with its schema version, applies the additive column and cleanup migrations that remain, and opens it. It refuses a store written before the item/step split (one that still carries the `nodes` table) with a message naming the lightcycle release able to migrate it; migrate with that release first, then reopen. Migrations are idempotent. Stop the pool loop before upgrading, so the old engine is not running against a newly-migrated store; restart it after.
