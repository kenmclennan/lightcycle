...
- **Overview** - the goal's description as one scrolling document with headings and wiki-links rendered (see below), followed, when one has been generated by `lc goal refresh`, by a `State of play` header in the shared heading style (generated timestamp right-aligned, dim) and its text; nothing renders for it when absent. The header line above the tab strip carries the title, the hand-set status and the project; the goal id is a CLI handle and is not shown.
