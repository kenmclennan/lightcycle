...
- **Overview** - the goal's description as one scrolling document, followed, when one has been generated by `lc goal refresh`, by a `State of play` header (generated timestamp right-aligned) and its text; nothing renders for it when absent. The header line above the tab strip carries the title, the hand-set status and the project; the goal id is a CLI handle and is not shown.
