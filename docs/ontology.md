...
- **project** - a free-text grouping label on an item (`Node.project`), independent of `repo`: one project may span several repos. Set via `lc new --project` (which also resolves the value against the project registry below, to pick an id shortcode) or `lc set --project` (which does not re-validate it against the registry). Every "PROJECT" column and `--project` filter reads this field. Distinct from the registry's own use of the word "project" (below) and from the workflow `workspace: project` keyword (see "The workflow"), which means "this stage's own `repo`" and is unrelated to this field.
