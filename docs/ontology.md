...
- **reopen** - undo a closed **item** outright (`lc reopen <item>`), clearing its outcome and close time so its state rolls up fresh from its children again. It does not, by itself, put the item back to work: a childless item rolls up to `backlogged` immediately, but one whose old children are still done still reports `done` until something changes beneath it - resuming it is filing a step directly against it, or, only if it never had any children, reactivating it (`lc set <item> --state active`), a separate action either way. Refuses a **step** (`--state ready` hands one back to its lane instead), a non-item node, or an item that isn't closed.
...
- **suspended** - a worker frozen with `SIGSTOP` under sustained memory pressure rather than killed; its step's `state` is unchanged (still `running`), the stall sweep excludes it while suspended, and it resumes with `SIGCONT` once pressure eases, with a fresh `stall-seconds` window from the moment it resumes.
...
- **project** - a registered codebase lightcycle files work against: an **identity**, a **shortcode**, a local path, and a remote URL. Stored in the project registry, managed with `lc project add|list|rm`.
  ...
  Confusingly shares its name with an item's own `project` field (see "The model (nouns)"): a registered project here is always one repo; an item's `project` is a human-chosen label that may span several. `lc new --project` is the only place the two meet - it resolves the value against this registry to pick a shortcode, then stores it verbatim as the item's `project` regardless. Nothing else assumes an item's `project` corresponds to a registered entry.
