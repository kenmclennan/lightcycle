...
- **role** - who processes the node: `agent` or `human`. A ready step with `role=human` is a human gate (it shows in the inbox); the state is still just `ready`. The stage a step performs is its `step` field.
...
- **artifacts** - typed values attached to an **item**: `repo`, `spec`, `branch`, `pr`, `feedback`, `retro`. Steps declare `accepts` / `produces` in their frontmatter, and the engine checks the item's artifacts against that contract before a step may close (e.g. `write-code` produces `branch`).
